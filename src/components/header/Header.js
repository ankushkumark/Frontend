...
// ✅ Apply dark mode jab bhi darkMode change ho
useEffect(() => {
...
<<<<<<< HEAD
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/recent-transactions/${userId}`
      );
      const data = await res.json();
  ...
  return (
    <header
      className={`bg-gray-100 dark:bg-gray-900 border-b border-gray-300 sticky top-0 z-40 transition-all duration-300 ${
